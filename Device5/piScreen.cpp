...
void piScreenCls::func() {
  switch (mode) {
    case 0:
      if (set && !counting) mode = 2,	digitalWrite(pin, HIGH), endF(12000), counting = true;
      break;
    case 1:
      if (!counting) {
...
void piScreenCls::confirm(boolean IO) {
  ...
  else if (!IO && mode == 3) counting = false, cnt = 0, mode = 4, endF(9000), counting = true;
}
