...
int udpLeesL = 0;
int dodTelMes = 0;
byte hetadres = 5; // zoek naar verander
byte mac[] = {
  0xDE, 0xBD, 0xBE, 0xEF, 0xFE, 0xEC
};
byte dimNu = 0;
IPAddress ip(192, 168, 179, 207);
...
piScreenCls piScreenC(9, 1);

void setup() {
  knopC.knopPoint = serKn.knop;
  pinMode (piezopin, OUTPUT);
  pinMode (A0, OUTPUT), digitalWrite(A0, LOW);
  pinMode (pinlamp[0], OUTPUT);
  analogWrite(pinlamp[0], 0);
...
boolean regulieruitvoergelezen () {
  return ((lees[0] == 'U' and lees[1] == 'V') || (lees[0] == 'V' and lees[1] == 'U'));
}

boolean tijdgelezenproc(char* lees, int leesGetelt) {
  return (leesGetelt > 3 &&  (lees[0] == 'T' || lees[0] == 't') and (lees[1] == 'T' or lees[1] == 'U' or lees[1] == 't' || lees[1] == 'u'));
}

boolean tempgelezenproc() {
  return (lees[0] == 'T' and lees[1] == 'E');
}

boolean SWgelezenproc() {
  return (lees[0] == 'S' and lees[1] == 'W');
}

boolean PSgelezenproc() {
  return (lees[0] == 'P' && lees[1] == 'S');
}

int leesnaaractie(char thisChar) {
  if (!leesstandaardproc(thisChar)) return 0;
  if (regulieruitvoergelezen ()) return 1;
  if (tijdgelezenproc(lees, leesGeteld)) return 2;
  if (tempgelezenproc()) return 3;
  if (SWgelezenproc()) return 4;
  if (PSgelezenproc()) return 5;
}
...
void verwerkcode(int switchvar, char* lees, byte leesaanv) {
  ...
  switch (switchvar) {
    case 1:
      if ((depin = uitvoerpin()) == 10) break;
      if (depin == 9) piScreenC.set = aanuitstuitv();
...
int uitvoerpin () {
  ...
  if (lees[6] == 'L' and lees[7] == 'A' and lees[8] == '0' and lees[9] == '0') {
  ...
  else if (lees[6] == 'P' and lees[7] == 'S' and lees[8] == '0' and (lees[9] == '0' || lees[9] == '1')) {
    uitkomst = 10;
  }
    ...
    uitkomst = (lees[8] - '0') * 10 + (lees[9] - '0');
  }
  else if (lees[6] == 'P' and lees[7] == 'I' and lees[8] == '0' and lees[9] - '0' < 4) uitkomst = 9;
  return uitkomst;
}
...
void loop() {
  telnetloop(), serKn.procKnop(), dimCheck = knopC.deLoop() || dimCheck;
  if (dimCheck && !delayWacht) {
    dimCheck = false;
...
void dodF() {
  ...
  } else tel = dodTel, dodTel = 0, dodBool2 = true, x();
  {
    for (unsigned int dTel = 0; dTel <= tel; dTel++) {
      ...
      }, NULL), knopledupdatetimer(), dimproc(tel - dTel), knopC.knipDoorF(tel - dTel);
    }
  }
...
void dodeman (int tDiff, boolean tijdup) {
  ...
  serKn.timF(), knopC.timerloop(tDiff);
  if (tijdup) {
